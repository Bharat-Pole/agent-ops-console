...
async def register_agent(req: dict[str, Any]) -> dict[str, Any]:
    ...
    # R8 — the config arrives from the client and its `bound_tools` has never
    # been validated server-side. Sanitize BEFORE the row is written, so an
    # unbindable ref is never persisted even for an instant. Strip, don't
    # reject: see services/bound_tools_guard.py.
    config, stripped_tools = await guard_config(config)
    ...
    audit_event = await audit_repo.insert(
        {
            ...
            + (f" {len(stripped_tools)} tool ref(s) stripped — see bind_stripped." if stripped_tools else ""),
        }
    )

    # After the insert, so the audit event and the `blocked` tool-call rows have
    # a real agent to point at. Silent when nothing was stripped.
    await record_stripped(new_id, stripped_tools, actor_persona)
    ...
    return {
        ...
        # The caller adopts `agent` wholesale, so the client self-corrects to the
        # sanitized config; this list is what it needs to *explain* the change.
        "strippedTools": stripped_tools,
    }
