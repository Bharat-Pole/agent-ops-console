...
from app.services.bound_tools_guard import guard_config, record_stripped
from app.services.connector_resolution import resolve_agent_connectors
...
#
# R8: this accepts a whole `config`, so it is a second door onto `bound_tools`
# and gets the same guard as registration.
#
# It reports and audits every strip, with no de-duplication. An earlier draft
# suppressed "repeat" syncs, on the assumption that the client re-sends an
# unsanitized config on each provisioning step. It does not: `syncAgentToServer`
# reads from the store, and the store adopted the server's sanitized agent at
# registration — so a repeat sync carries nothing to strip and is silent for
# free. Worse, the suppression could not tell a *first* attempt from a repeat
# (both look like "sanitized result == stored"), so it silently swallowed the
# very evidence this guard exists to produce.
@router.patch("/v1/agents/{agent_id}")
async def patch_agent(agent_id: str, body: dict[str, Any]) -> JSONResponse:
    stripped: list[dict[str, Any]] = []
    if isinstance(body.get("config"), dict):
        body = {**body}
        body["config"], stripped = await guard_config(body["config"])
    ...
    await record_stripped(agent_id, stripped)
    return JSONResponse(content={"agent": agent, "strippedTools": stripped})
...
# The agent's MCP dependency set — which connectors its bound tools resolve to,
# which tools are local and need none, and which connectors are unhealthy.
# Pure derivation; see services/connector_resolution.py.
@router.get("/v1/agents/{agent_id}/connectors")
async def agent_connectors(agent_id: str) -> JSONResponse:
    result = await resolve_agent_connectors(agent_id)
    if result is None:
        return JSONResponse(status_code=404, content={"message": "Agent not found."})
    return JSONResponse(content=result)
