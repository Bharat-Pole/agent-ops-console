...
# 6. Run client + server ------------------------------------------------
# NOTE: do not use `npm run dev` here. Its `dev:server` script uses a
# forward-slash venv path that cmd.exe cannot resolve, so the server leg dies
# with "'backend' is not recognized" while the client leg comes up anyway —
# you get a working page talking to nothing. Start the two legs explicitly.
Write-Step "Starting backend (http://localhost:8787)"
$backend = Start-Process -FilePath $venvPython `
    -ArgumentList "-m", "uvicorn", "app.main:app", "--app-dir", "backend", "--reload", "--port", "8787" `
    -PassThru -NoNewWindow

Write-Step "Starting client (http://localhost:5173)"
try {
    npm run dev:client
}
finally {
