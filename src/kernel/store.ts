...
export interface WorkspaceState extends WorkspaceData {
  ...
  // Overwrites the server-owned slices with persisted truth from GET /v1/bootstrap.
  hydrateFromServer: (data: { agents: AgentRecord[]; approvals: ApprovalItem[]; auditLog: AuditEvent[]; evalPacks: EvaluationPack[]; knowledgeSources?: RealKnowledgeSource[] }) => void;
  // Real knowledge sources from the backend DB
